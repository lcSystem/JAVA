...
public class UserService implements UserDetailsService {
    ...
    @Autowired
    private com.allianceever.projectERP.AuthenticatedBackend.repository.RoleMenuPermissionRepository roleMenuPermissionRepository;
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        ...
        com.allianceever.projectERP.AuthenticatedBackend.models.ApplicationUser user = userRepository
                .findByUsername(username)
                .orElseThrow(() -> new UsernameNotFoundException("user is not valid"));

        // Load permissions and add as authorities
        java.util.Set<Integer> roleIds = user.getAuthorities().stream()
                .map(auth -> ((com.allianceever.projectERP.AuthenticatedBackend.models.Role) auth).getRoleId())
